...
Note that DCP is a special case of $\edcp_{n, q, r}$ where $n = 1$, $q = N$ and $r = 2$. Conversely, $\edcp$ can be reduced to vectorial variant of DCP which can be solved using the same technique as in \cite{kuperberg2005subexponential}. We briefly explain the steps of the algorithm here. First, given the distribution $\mu_{\bm{s}, r}$ we can efficiently sample from the distribution $\mu_{\bm{s}, 2}$ using Lemma \ref{lem:self-rd}. A sample from $\mu_{\bm{s}, 2}$ is of the form
...
\begin{equation}
    \label{equ:dih-sve}
    \ket{\phi_{\bm{y}}} = \frac{1}{\sqrt{2}}(\ket{0} + \omega_q^{ys_n}\ket{1}).
\end{equation}
Suppose we have $t \in O(\log q)$ of the states \eqref{equ:dih-sve}. Then
...
where $\alpha(x) = x_0y_0 + \cdots + x_ty_t$. 
