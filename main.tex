...
%% \[ \ket{\phi_{\bm{s}, \bm{x}, b}} = \frac{1}{\sqrt{r}} \sum_{j = 0}^{r - 1} (-1)^{bj}\ket{j}\ket{\bm{x} + j\bm{s}} \]
%% for some uniformly random $\bm{x} \in \Z_q$. Then the  system is described by the density operator  
%% \[ \Phi_{\bm{s}, b} = \frac{1}{q^n} \sum_{\bm{x} \in \Z_q^n} \ket{\phi_{\bm{s}, \bm{x}, b}} \bra{\phi_{\bm{s}, \bm{x}, b}}. \]
...
%%     \[ U_1 \ket{j}\ket{\bm{x} + j\bm{s}} = (-1)^{x_1 + js_1} \ket{j}\ket{\bm{x} + j\bm{s}}. \]
%%     Ignoring the global phase $(-1)^{x_1}$, $U_1$ acts on the state $\ket{\phi_{\bm{s}, \bm{x}, 0}}$ as follows
%%     \[ U_1 \ket{\phi_{\bm{s}, \bm{x}, 0}} = \frac{1}{\sqrt{r}} \sum_{j = 0}^{r - 1} (-1)^{js_1}\ket{j}\ket{\bm{x} + j\bm{s}}. \]
%%     This means we have
%%     \begin{equation}
%%         \label{equ:u1}
...
%%     \[ U_{k + 1} \ket{j}\ket{\bm{x} + j\bm{s}} = \omega_{2^{k + 1}}^{x_1 + js_1} \omega_{2^{k + 1}}^{-j\tilde{s}_1} \ket{j}\ket{\bm{x} + j\bm{s}}. \]
...
\item Apply $\qft_r \otimes \mathds{1}$ to the register $\ket{0} \ket{\bm{x}}$ to obtain the state $\frac{1}{\sqrt{r}} \sum_{j = 0}^{r - 1} \ket{j} \ket{\bm{x}}$.
\item Apply the the operation $\ket{j}\ket{\bm{x}} \mapsto \ket{j}\ket{\bm{x} + j\bm{s}}$ to obtain the state $\ket{\phi_{\bm{s}, \bm{x}, 0}} = \frac{1}{\sqrt{r}} \sum_{j = 0}^{r - 1} \ket{j} \ket{\bm{x} + j\bm{s}}$
\item Publish the state $\ket{\phi_{\bm{s}, \bm{x}, 0}}$.     
\end{enumerate}
...
\item To encrypt a bit $b \in \{ 0, 1 \}$, Alice performs the unitary $U: \ket{j}\ket{\bm{y}} \mapsto (-1)^{bj}\ket{j}\ket{\bm{y}}$ on $\Phi_{\bm{s}, 0}$ to obtain the state $\Phi_{\bm{s}, b}$.
...
\begin{enumerate}[topsep = 1pt, itemsep = 1pt, parsep = 1pt]
\item Apply the the unitary $U: \ket{j}\ket{\bm{y}} \mapsto \ket{j}\ket{\bm{y} - j\bm{s}}$ to $\Phi_{\bm{s}, b}$.
...
\begin{align*}
    \Phi_{\bm{s}, b}
    & \mapsto U \frac{1}{q^n} \sum_{\bm{x} \in \Z_q^n} \ket{\phi_{\bm{s}, \bm{x}, b}} \bra{\phi_{\bm{s}, \bm{x}, b}} U^*  \tag{apply $U$} \\
    & = \frac{1}{q^n} \sum_{\bm{x} \in \Z_q^n} \frac{1}{r} \sum_{j = 0}^{r - 1} \sum_{k = 0}^{r - 1} (-1)^{b(j - k)}\ket{k}\ket{\bm{x}} \bra{j}\bra{\bm{x}} \\
    & = \frac{1}{r} \sum_{j = 0}^{r - 1} \sum_{k = 0}^{r - 1} (-1)^{b(j - k)}\ket{k}\bra{j} \otimes \frac{1}{q^n} \sum_{\bm{x} \in \Z_q^n} \ket{\bm{x}}\bra{\bm{x}} \\
    & \mapsto \frac{1}{r} \sum_{j = 0}^{r - 1} \sum_{k = 0}^{r - 1} (-1)^{b(j - k)}\ket{k}\bra{j} \tag{discard the second register} \\
    & \mapsto \qft_r \frac{1}{r} \sum_{j = 0}^{r - 1} \sum_{k = 0}^{r - 1} (-1)^{b(j - k)}\ket{k}\bra{j} \qft_r^* \tag{apply quantum Fourier transfom}\\
    & = \ket{br/2} \bra{br/2}
\end{align*}
